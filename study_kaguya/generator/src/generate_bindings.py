...
class FunctionInfo(object):
    ...
    def __init__(self, fn_cursor):
        ...
        if fn_cursor.kind == CursorKind.CXX_METHOD and fn_cursor.is_static_method(
        ) == False:
            class_cursor = fn_cursor.lexical_parent
            classname = namespace_name(
                class_cursor) + "::" + class_cursor.spelling
            self.__pointer_type_name = "%s (%s::*)(%s)" % (
                self.result_type_name, classname, ','.join(arg_types))
            if fn_cursor.is_const_method():
                self.__pointer_type_name = self.__pointer_type_name + " const"
        else:
            self.__pointer_type_name = "%s (*)(%s)" % (self.result_type_name,
                                                       ','.join(arg_types))
    ...
    def is_template_specialized(self):
        if clang.cindex.conf.lib.clang_getSpecializedCursorTemplate(
                self.__cursor):
            return True
...
class ClassInfo(object):
    ...
    def __init__(self, cursor):
        assert (cursor.kind == CursorKind.STRUCT_DECL or cursor.kind ==
    ...
    def get_pure_virtual_functions(self):
        ...
        for omfn in list(self.medhods()) + list(self.operators()):
            for mfn in omfn:
...
class BindingData(object):
    ...
    def __init__(self, translation_unit):
        self.__function_list = self.export_function_map(
            translation_unit.cursor)
        self.__class_list = self.export_class_list(translation_unit.cursor)
        self.__constant_field_list = self.export_constant_field_list(
            translation_unit.cursor)
    ...
    def is_class_export(cls, class_cursor):
        return class_cursor.is_definition() and (
            class_cursor.kind == CursorKind.STRUCT_DECL or
            class_cursor.kind == CursorKind.CLASS_DECL)
    ...
    def is_constant_field_export(cls, class_cursor):
        return class_cursor.is_definition() and (
            class_cursor.kind == CursorKind.ENUM_CONSTANT_DECL)
...
class ExportFilter:
    ...
    def filter_name(self, name):
        return self.patterns_match(
            name, self.__match_patterns) and not self.patterns_match(
                name, self.__ignore_patterns)

    def filter_function(self, data):
        return data.is_public_member() and self.patterns_match(
            data.displayname,
            self.__match_patterns) and not self.patterns_match(
                data.displayname, self.__ignore_patterns)


class ExportFilters:
    def __init__(self, conf):
        base_filter = ExportFilter(conf)
        self.function_filter = ExportFilter(
            conf.get('function'), base_filter).filter_function
        self.class_filter = ExportFilter(conf.get('class'),
                                         base_filter).filter_function
        self.class_member_filter = ExportFilter(
            conf.get('class_member'), base_filter).filter_function
        self.constant_filter = ExportFilter(
            conf.get('constant'), base_filter).filter_function
        self.stringify_class = ExportFilter(
            conf.get('stringify_class'), base_filter).filter_function
...
class BindingWrapperWriter(BindingWriter):
    ...
    def __export_defarg_function(self, fnc, name, minarg, maxarg):
        if fnc.is_static_method():
            self.file.write(
                "KAGUYA_FUNCTION_OVERLOADS_WITH_SIGNATURE( %s, %s,  %d, %d, %s);\n"
                % (name, fnc.name, minarg, maxarg, fnc.pointer_type_name))
        else:
            self.file.write(
                "KAGUYA_MEMBER_FUNCTION_OVERLOADS_WITH_SIGNATURE( %s , %s , %s,  %d, %d, %s);\n"
                % (name, fnc.namespace, fnc.function_name, minarg, maxarg,
                   fnc.pointer_type_name))
    ...
    def __write_wrapper_function(self, fnc):
        if fnc.default_parameter_count > 0:
            ...
            self.__export_defarg_function(fnc, fnc.function_name + "_wrap_obj",
                                          minparam_count, maxparam_count)
    ...
    def __write_wrapper_overloads_fnction(self, fncs):
        ...
        for index, fnc in enumerate(fncs):
            ...
            if fnc.default_parameter_count > 0:
                maxparam_count = fnc.parameter_count
                minparam_count = fnc.parameter_count - fnc.default_parameter_count
                self.__export_defarg_function(
                    fnc, fnc.function_name + "_wrap_obj" + str(index),
                    minparam_count, maxparam_count)
                self.file.write("auto %s = %s();\n" % (
    ...
    def write_wrapper(self, binding_data, wrapper_namespace_prefix, filters):
        for fncs in binding_data.function_list():
            fncs = list(filter(filters.function_filter, fncs))
            if len(fncs) == 0:
                continue
        ...
        for clss in filter(filters.class_filter, binding_data.class_list()):
            for method in clss.medhods():
                method = list(filter(filters.class_member_filter, method))
                if len(method) == 0:
                    continue
                self.begin_namespace(method[0].namespace,
                                     wrapper_namespace_prefix)
                if len(method) == 1:
    ...
    def __out_class_methods(self, clss, filterf):
        for method in clss.medhods():
            method = list(filter(filterf, method))
            if len(method) == 0:
                continue
            if method[0].is_static_method():
                self.file.write('    .class_function("%s", %s)\n' % (
                    method[0].function_name, insert_prefix_to_namespace(
                        clss.name, wrapper_namespace_prefix) + "::" +
                    method[0].function_name))
            else:
    ...
    def __out_class_operators(self, clss, filterf):
        for opera in clss.operators():
            opera = list(filter(filterf, opera))
            if len(opera) == 0:
                continue
            metamethodname = self.__operator_bind_map.get(opera[0]
                                                          .function_name)
            ...
            if len(opera) == 1:
                ...
                overloads = []
                for op in opera:
                    overloads.append('(%s)&%s' %
                                     (op.pointer_type_name, op.name))
                self.file.write(
                    '    .class_function("%s", kaguya::overload(%s))\n' %
    ...
    def __out_class_def(self, clss, filters):
        if clss.base_class_count == 1:
            ...
            self.file.write('  class_<%s,kaguya::MultipleBase<%s> >("%s")\n' %
                            (clss.name, ','.join(clss.base_class_names()),
                             clss.class_name))
        else:
        ...
        if filters.stringify_class(clss):
            self.file.write(
                '    .class_function("__tostring", &kaguya_stringifier<%s>)\n'
                % (clss.name))
        self.file.write('  ;\n')

    def write_binding(self, binding_data, wrapper_namespace_prefix,
                      manual_binding_file_name, filters):
        ...
        for fncs in binding_data.function_list():
            ...
            if len(fncs) > 0:
                self.file.write('  function("%s", %s);\n' % (
                    fncs[0].function_name, insert_prefix_to_namespace(
                        fncs[0].namespace, wrapper_namespace_prefix) + "::" +
                    fncs[0].function_name))
        for clss in filter(filters.class_filter, binding_data.class_list()):
            self.__out_class_def(clss, filters)

        for field in filter(filters.constant_filter,
                            binding_data.constant_field_list()):
    ...
    def write(self, binding_data, wrapper_namespace_prefix,
              manual_binding_file_name, filters):
        ...
        self.write_binding(binding_data, wrapper_namespace_prefix,
                           manual_binding_file_name, filters)
...
if __name__ == '__main__':
    ...
    if len(args) > 1:
        ...
        souce_file = "/home/heaven7/heaven7/libs/opencv-3.4.7/opencv-4.5.4/_install/include/opencv4/opencv2/opencv.hpp"
    output = sys.stdout
    ...
    if souce_ext == '.ast':
        ...
        tu = index.parse(souce_file, clang_args, None,
    ...
    binding_writer.write(bindinf_data, wrapper_namespace_prefix,
                         manual_binding_file_name, bfilter)
